...
export default function Step6WebsiteType({ data, update, stepNumber, totalSteps, fieldErrors = {} }: { data: BriefData; update: (p: any) => void; stepNumber: number; totalSteps: number; fieldErrors?: FieldErrors }) {
  const TYPES = [
      ...
      <div className="space-y-10">
        <div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {TYPES.map(t => (
              <button key={t.id} type="button" onClick={() => update({ websiteType: t.id })}
                className={`p-6 rounded-2xl border-2 text-left transition-all ${
                  data.websiteType === t.id
                    ? 'border-black bg-apple-gray-50'
                    : fieldErrors.websiteType
          ...
          {fieldErrors.websiteType && (
            <p className="mt-3 text-sm text-red-600">{fieldErrors.websiteType}</p>
          )}
          {data.websiteType === 'custom' && (
            <div className="mt-6">
              <label className="block text-sm font-medium text-apple-gray-500 mb-3">
                Jelaskan jenis website custom Anda <span className="text-apple-gray-400 font-normal">(opsional)</span>
              </label>
              <textarea value={data.websiteTypeOther || ''} onChange={e => update({ websiteTypeOther: e.target.value })}
                placeholder="Ceritakan detail jenis website yang Anda inginkan..." rows={3}
        ...
        <div>
          <label className="block text-xl font-semibold mb-2">
            Tujuan Website <span className="text-base font-normal text-apple-gray-400">(opsional)</span>
          </label>
          ...
          <div className="flex flex-wrap gap-3">
            {PURPOSES.map(p => (
              <button key={p} type="button" onClick={() => togglePurpose(p)}
                className={`px-5 py-3 rounded-full font-medium transition-all ${(data.websitePurpose || []).includes(p) ? 'bg-black text-white' : 'bg-apple-gray-100 text-apple-gray-600 hover:bg-apple-gray-200'}`}>
                {p}
              </button>
            ))}
          </div>
          {hasOtherPurpose && (
            <textarea value={data.websitePurposeOther || ''} onChange={e => update({ websitePurposeOther: e.target.value })}
              placeholder="Sebutkan tujuan website lainnya..." rows={2}
